...
void z80cpu::SBC_implied_register() {
    ...
    // A = A - D - C
    // A = A - (D + C)
    uint8_t data = *register_table[opcode & BIT_MASK_2] + get_flag(CARRY_FLAG);
    uint8_t result = accumulator - data;
...
void z80cpu::SBC_implied_register_indirect() {
    ...
    // A = A - (D + C)
    address_absolute = (static_cast<uint16_t>(H_register) << 8) | L_register;
    uint8_t data = ram_read(address_absolute) + get_flag(CARRY_FLAG);
    uint8_t result = accumulator - data;
...
void z80cpu::SBC_implied_immediate() {
    ...
    uint8_t data = rom_read(program_counter) + get_flag(CARRY_FLAG);
    program_counter++;
    uint8_t result = accumulator - data;
